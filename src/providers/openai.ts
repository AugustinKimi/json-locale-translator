...
export class OpenAIProvider implements TranslationProvider {
  ...
  async translate(
    ...
    try {
      ...
      // Retry once
      process.stderr.write(`[warn] Failed to parse OpenAI response as JSON, retrying...\n`)
      let raw2: string
      try {
        raw2 = await callApi()
      } catch (err) {
        throw new Error(`OpenAI API call failed on retry: ${err instanceof Error ? err.message : String(err)}`)
      }
      try {
        parsed = JSON.parse(raw2)
      } catch {
        throw new Error(`OpenAI response is not valid JSON after retry. Raw response:\n${raw2}`)
      }
    }
    ...
    for (const [key, originalValue] of Object.entries(keys)) {
      if (key in parsed) {
        ...
        process.stderr.write(`[warn] Missing key "${key}" in OpenAI translation response, keeping original value\n`)
